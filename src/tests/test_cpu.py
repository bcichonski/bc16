...
class CpuTests(unittest.TestCase):
    ...
    def test_CLC16_logical_opcodes(self):
        if debug: print("test_CLC16_logical_opcodes")
        #given
        cpu = self.create_cpu([
            0x18, 0xAA,                   # 0x0000: MOV CS, 0xAA ; stack at 00fe
            0x14, 0xAA,                   # 0x0002: MOV CI, 0xAA
            0x19, 0x55,                   # 0x0004: MOV DS, 0x55
            0x15, 0x55,                   # 0x0006: MOV DI, 0x55
            0x5f, 0xbb, 0x80,             # 0x0008: OR CSCI, DSDI
            0x5f, 0xb2, 0x80, 0xFF, 0x00, # 0x000b: AND CSCI, 0xFF00
            0x5f, 0xba, 0x80,             # 0x000f: AND CSCI, DSDI
            0x5f, 0xb3, 0x80, 0x00, 0x50, # 0x0013: OR CSCI, 0x0050
            0x5f, 0xbc, 0x80,             # 0x0016: XOR CSCI, DSDI
            0x5f, 0xb4, 0x80, 0xFF, 0xFF, # 0x0019: XOR CSCI, 0xFFFF
            0x5f, 0xb6, 0x90, 0x00, 0x0c, # 0x0020: SHR DSDI, 0x000c
            0x5f, 0xb5, 0x90, 0x00, 0x01, # 0x0020: SHL DSDI, 0x0001
            0x5f, 0xbe, 0x80,             # 0x001d: SHR CSCI, DSDI
            0x5f, 0xbd, 0x80,             # 0x001d: SHL CSCI, DSDI
            0x5f, 0xb7, 0x80,             # 0x0024: NOT CSCI
            0xff                          # 0x0010: KIL
        ])
        #when
        cpu.run()
        #then
        self.assertEqual(cpu.cs.get(),  0x03)
        self.assertEqual(cpu.ci.get(),  0xff)

    def test_JMR16_opcodes(self):
        if debug: print("test_JMR16_opcodes")
        #given
        cpu = self.create_cpu([
            0x11, 0x00,            # 0x0000: MOV CS, 0x00 ; stack at 00fe
            0x14, 0x02,            # 0x0002: MOV CI, 0x02
            0xe2, 0x08,            # 0x0004: JMR nz, CSCI
            0xff,                  # 0x0006: KIL
            0xe1, 0x00, 0x00, 0x02,# 0x0007: JMR nz, +2
            0xff,                  # 0x000b: KIL
            0x18, 0xab,            # 0x000c: MOV CS, 0xab
            0xff,                  # 0x000d: KIL
        ])
        #when
        cpu.run()
        #then
        self.assertEqual(cpu.cs.get(),  0xab)
