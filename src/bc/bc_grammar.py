...
expression_functioncall = seq(function_name = ident, params = ignore >> leftpar >> ignore >> nl.optional() >> ignore >> expression.sep_by(nl.optional() >> comma << nl.optional()) << ignore << nl.optional() << ignore << rightpar << ignore).combine_dict(EXPRESSION_CALL).desc("function call")
expression_nested = (leftpar >> ignore >> expression << ignore << rightpar).desc('nested expression')
expression_term = (expression_nested | conststring | constnumber | expression_functioncall | ident).map(EXPRESSION_TERM).desc('term expression')
...
expression_logic = seq(operand1 = expression_bincal, arguments = seq(binary_logic, expression_bincal).many()).combine_dict(EXPRESSION_BINARY).desc('binary expression logical')
expression_comparision = seq(operand1 = expression_logic, arguments = seq(binary_comp, expression_logic).many()).combine_dict(EXPRESSION_BINARY).desc('binary expression bitwise shifts')
